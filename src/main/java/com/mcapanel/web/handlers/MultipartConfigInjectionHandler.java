 ...
 * something with the multipart parts, as the saved parts will be deleted on the
 * return from
 * {@link #handle(String, Request, HttpServletRequest, HttpServletResponse)}.
 */
public class MultipartConfigInjectionHandler extends HandlerWrapper
{
	...
	/**
	 * If you want to have multipart support in your handler, call this method
	 * each time your doHandle method is called (prior to calling getParameter).
	 *
	 * Servlet 3.0 include support for Multipart data with its
	 * {@link HttpServletRequest#getPart(String)} &
	 * {@link HttpServletRequest#getParts()} methods, but the spec says that
	 * before you can use getPart, you must have specified a
	 * {@link MultipartConfigElement} for the Servlet.
	 *
	 * <p>
	 * This is normally done through the use of the MultipartConfig annotation
	 * of the servlet in question, however these annotations will not work when
	 * specified on Handlers.
	 *
	 * <p>
	 * The workaround for enabling Multipart support in handlers is to define
	 * the MultipartConfig attribute for the request which in turn will be read
	 * out in the getPart method.
	 *
	 * @see <a
	 *      href="https://bugs.eclipse.org/bugs/show_bug.cgi?id=395000#c0">Jetty
	 *      Bug tracker - Jetty annotation scanning problem (servlet workaround)
	 *      </a>
	 * @see <a
	 *      href="http://dev.eclipse.org/mhonarc/lists/jetty-users/msg03294.html">Jetty
		...
		} finally
		{
			if (multipartRequest)
			{
				...
				if (multipartInputStream != null)
				{
					try
					{
						// a multipart request to a servlet will have the parts
						// cleaned up correctly, but
						// the repeated call to deleteParts() here will safely
						// do nothing.
						multipartInputStream.deleteParts();
					} catch (MultiException e)
					{
						// LOG.error("Error while deleting multipart request parts",
						// e);
					}
				}
			}
